...
**Check your own code:** every file in `soroban-zk-core` and `soroban-zk-std` must begin with `#![no_std]`. If this attribute is missing, the compiler assumes `std`.
...
```bash
cargo test -p soroban-zk-std -- measure_my_function --nocapture
```
...
`soroban-zk-core` contains only pure field arithmetic, which needs nothing from `std`. `soroban-zk-std` depends on the Soroban SDK, which provides its own `no_std`-compatible allocator when needed.
...
| A developer tool for use inside contracts | `crates/soroban-zk-std/` — may use `soroban_sdk` |
...
```bash
make build-wasm
twiggy top target/wasm32-unknown-unknown/release/soroban_zk_std.wasm
```
